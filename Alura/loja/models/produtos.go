...
import (
	"github.com/fabiossilva00/Go/Alura/loja/db"
)
...
func CriarProduto(nome, descricao string, preco float64, quantidade int) {
	db := db.ConectaBancoPostgres()
	insertPayload, err := db.Prepare("insert into produtos(nome, descricao, preco, quantidade) values ($1, $2, $3, $4)")
	if err != nil {
		panic(err.Error())
	}
	insertPayload.Exec(nome, descricao, preco, quantidade)

	defer db.Close()
}

func ExcluiProduto(idProduto int) bool {
	db := db.ConectaBancoPostgres()
